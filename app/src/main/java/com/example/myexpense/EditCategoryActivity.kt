...
class EditCategoryActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        oldName = intent.getStringExtra("CATEGORY_NAME") ?: ""
        selectedIcon = intent.getStringExtra("CATEGORY_ICON") ?: "ic_others"
        selectedColor = intent.getStringExtra("CATEGORY_COLOR") ?: "#757575"

        etName.setText(oldName)

        findViewById<ImageView>(R.id.iv_back).setOnClickListener {
            showDiscardDialog(etName.text.toString())
        }

        findViewById<ImageView>(R.id.iv_header_delete).setOnClickListener {
        ...
        findViewById<Button>(R.id.btn_save_changes).setOnClickListener {
            ...
            if (newName.isNotEmpty()) {
                ...
                val updatedCategory = Category(newName, selectedIcon, selectedColor)
                if (db.updateCategory(userId, oldName, updatedCategory)) {
    ...
    private fun showDiscardDialog(currentName: String) {
        // Simple check if anything changed
        val anythingChanged = currentName != oldName || 
                             selectedIcon != intent.getStringExtra("CATEGORY_ICON") ||
    ...
    private fun showDeleteConfirmationDialog() {
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Confirm") { _, _ ->
                val userId = session.getUserId()
                if (db.deleteCategory(userId, oldName)) {
                    finish()
                }
            }
